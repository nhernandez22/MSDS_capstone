---
title: 'Designing a Route Finder App Using Oregon Climbing Data'
author: "Nina"
date: '2022-05-30'
output:
  html_document:
...
```
      Looking at the state level, we find that a greater percentage of Oregon routes fall into the "Area Classic" range with outliers in the "Bad" to "Bomb" class. But what about at the grade level?
```{r, echo = FALSE}
or_ratings %>%
  group_by(state) %>%
  ggplot(aes(x = 0, y = ARQI_median, fill = state)) +
  geom_boxplot() +
  scale_fill_brewer(palette = "Set2") + 
  labs(x = "", y = "ARQI\n", title = "Distribution of Oregon ARQI Scores\n") +
  theme_tufte() +
  theme(legend.position = "none",
        axis.ticks.x = element_blank(),
        axis.text.x = element_blank(),
        plot.background = element_rect(fill = 'grey90')) +
  geom_hline(yintercept = 3.5, linetype = "dashed", color = "#FFFFB2") +
  geom_hline(yintercept = 2.5, linetype = "dashed", color = "#FECC5C") +
  geom_hline(yintercept = 1.5, linetype = "dashed", color = "#FD8D3C") +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "#BD0026") +
  annotate(x = 0.1, y = 3.7, geom = "text", label = "Classic", size = 2, family = 'serif') +
  annotate(x = 0.1, y = 2.7, geom = "text", label = "Area Classic", size = 2, family = 'serif') +
  annotate(x = 0.1, y = 1.7, geom = "text", label = "Good", size = 2, family = 'serif') +
  annotate(x = 0.1, y = .7, geom = "text", label = "Bad", size = 2, family = 'serif') +
  annotate(x = 0.1, y = 0, geom = "text", label = "Bomb", size = 2, family = 'serif')
```

      A majority of easy and intermediate routes are both classic and area classics, which supports my claim that anyone can climb a classic not only at their local crag but additionally famous big walls climbed by the legends. 
